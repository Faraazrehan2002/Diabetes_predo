...
import pandas as pd #to convert input data into a dataframe for giving as a input to the model
...
def predict():
    ...
    if output>str(0.5):
        return render_template('index.html',pred='You have chance of having diabetes.\nProbability of having Diabetes is {}'.format(output)) #Returning the message.
    else:
        return render_template('index.html',pred='Seems like you safe.\nProbability of having Diabetes is {}'.format(output))
